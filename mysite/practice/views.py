...
######################################################################################################################
## 정확도 표출
def accuracy(request,st_cd='0'):
    finacedata = []
    cursor = connection.cursor()
    if st_cd == '0':
        query2 = 'SELECT * FROM `accuracy`order by Date desc limit 5;'
        result = cursor.execute(query2)
        result = pd.DataFrame(cursor.fetchall())
        ...
        query2 = 'SELECT * FROM `accuracy` where st_cd = {} order by Date desc limit 1;'.format(st_cd)
        result = cursor.execute(query2)
        result = pd.DataFrame(cursor.fetchall())

        # row = {
        #     'st_cd': result.iloc[0][0],
        #     'news': str(result.iloc[0][1]),
        #     'datetime': str(result.iloc[0][2][:10]),
        #     'title': str(result.iloc[0][3]),
        #     'Tokenization': str(result.iloc[0][4]),
        #     'Positive_Score': str(result.iloc[0][5]),
        #     'Negative_Score': str(result.iloc[0][6]),
        #     'Ratio': str(result.iloc[0][7]),
        #     'Rred': str(result.iloc[0][8]),
        # }
    ...
    row = {
        'dates': result.iloc[0][0][:10],
        'asia': str(result.iloc[0][1]),
        'maeil': str(result.iloc[0][2]),
        'hk': str(result.iloc[0][3]),
        'sampro': str(result.iloc[0][4]),
        'suka': str(result.iloc[0][5]),
    }

    finacedata.append(row)
    accuracyJson = json.dumps(finacedata)
    return accuracyJson
################################
### index_정확도
